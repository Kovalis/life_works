  ...
  if (nextTab) {
    nextTab.forEach(function (el) {
      el.addEventListener("click", function (element) {
        element.preventDefault(); // проверка на заполнение полей перед переключением таба
        ...
        if ($activeInnerTab) {
          ...
          if (_$tabPanel.parents(".main-form-item").find(".form-error").hasClass("form-error_show")) {
            ...
            console.log("Для кнопок nextTab надо проверку сделать на незаполненные input(inputchange trigger) события вызвать");
            clickNextTab();
          }
        }
